...
#define SITUATION_APPROACH 0
#define SITUATION_SHOOT 1
#define NUMBER_STEPS_IN_SHOOT 200
...
///
class AgentTDSituative : public BaseAgent
{
public:
    AgentTDSituative();
	~AgentTDSituative();
    ...
    int policy(int state, int situation) const;
    ...
    int get_situation(const Eigen::Ref<const Eigen::VectorXd> &obs) const;
...
private:
    ...
    Eigen::MatrixXd Q_approach;
    Eigen::MatrixXd Q_shoot;
	double lr = 0.001;
	double eligibility = 0.7;
	double discount = 0.8;
	double eGreedy = 0.3;
    int last_random_action_approach = 0;
    int last_random_action_shoot = 0;
    int counter_random_action_approach = REPEAT_RANDOM_ACTION;
    int counter_random_action_shoot = REPEAT_RANDOM_ACTION;
    int nr_shoot_steps = NUMBER_STEPS_IN_SHOOT;
    bool just_shooted = false;
};
